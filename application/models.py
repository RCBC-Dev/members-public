...
class UserMapping(models.Model):
    ...
    """
    legacy_user = models.ForeignKey(User, on_delete=models.CASCADE, related_name='legacy_mappings_from', help_text="Original user (e.g., Kerry who left)")
    sso_user = models.ForeignKey(User, on_delete=models.CASCADE, related_name='legacy_mappings_to', help_text="New MS SSO user (e.g., 'KM1655')")
    ...
    applied_at = models.DateTimeField(null=True, blank=True, help_text="When this mapping was applied to historical data")
    ...
    def apply_to_enquiries(self):
        ...
        with transaction.atomic():
            ...
            new_admin, created = Admin.objects.get_or_create(user=self.sso_user)
            
            # Update enquiries assigned to legacy admin
            enquiry_count = 0
            if legacy_admin:
                enquiry_count = Enquiry.objects.filter(admin=legacy_admin).update(admin=new_admin)
            
            # Update enquiry history records created by legacy user
            history_count = EnquiryHistory.objects.filter(created_by=self.legacy_user).update(created_by=self.sso_user)
            
            # Update enquiry attachments uploaded by legacy user
            attachment_count = EnquiryAttachment.objects.filter(uploaded_by=self.legacy_user).update(uploaded_by=self.sso_user)
            
            # Update audit records for legacy user
            audit_count = Audit.objects.filter(user=self.legacy_user).update(user=self.sso_user)
            
            # Mark as applied if any data was transferred
            total_updates = enquiry_count + history_count + attachment_count + audit_count
            if total_updates > 0:
...
class Member(models.Model):
    ...
    class Meta:
        ...
        ordering = ['-is_active', 'first_name', 'last_name']  # Active members first, then alphabetical
        indexes = [
...
class Enquiry(models.Model):
    ...
    member = models.ForeignKey(Member, related_name='enquiries', on_delete=models.PROTECT)
    admin = models.ForeignKey(Admin, related_name='assigned_enquiries', null=True, blank=True, on_delete=models.SET_NULL)
    created_at = models.DateTimeField(default=timezone.now)
    updated_at = models.DateTimeField(auto_now=True)
    closed_at = models.DateTimeField(null=True, blank=True)
    section = models.ForeignKey(Section, related_name='enquiries', null=True, blank=True, on_delete=models.SET_NULL)
    contact = models.ForeignKey('Contact', related_name='enquiries', on_delete=models.SET_NULL, null=True, blank=True)
    job_type = models.ForeignKey(JobType, on_delete=models.SET_NULL, null=True, blank=True, related_name='enquiries')
    service_type = models.CharField(max_length=20, choices=SERVICE_TYPE_CHOICES, null=True, blank=True, db_index=True)
    ...
    class Meta:
        ...
        indexes = [
            ...
            models.Index(fields=['status', 'created_at'], name='enquiry_status_created_idx'),
            ...
            models.Index(fields=['member', '-created_at'], name='enquiry_member_created_idx'),

            # Section + created_at for section-specific queries
            models.Index(fields=['section', '-created_at'], name='enquiry_section_created_idx'),

            # Admin + created_at for assigned enquiries
            models.Index(fields=['admin', '-created_at'], name='enquiry_admin_created_idx'),

            # Contact + created_at for contact-specific queries
            models.Index(fields=['contact', '-created_at'], name='enquiry_contact_created_idx'),
            ...
            models.Index(fields=['job_type', '-created_at'], name='enquiry_jobtype_created_idx'),

            # Date range filtering indexes
            models.Index(fields=['created_at'], name='enquiry_created_asc_idx'),

            # Compound index for common filter combinations
            models.Index(fields=['status', 'member', '-created_at'], name='enq_status_member_created_idx'),
            models.Index(fields=['status', 'section', '-created_at'], name='enq_status_section_created_idx'),
            ...
            models.Index(fields=['reference', '-created_at'], name='enq_ref_created_idx'),

            # Service type filtering index
            models.Index(fields=['service_type', '-created_at'], name='enq_service_type_idx'),
...
class EnquiryHistory(models.Model):
    ...
    enquiry = models.ForeignKey(Enquiry, related_name='history', on_delete=models.PROTECT)
    ...
    class Meta:
        ...
        indexes = [
            ...
            models.Index(fields=['enquiry', '-created_at'], name='history_enquiry_created_idx'),
...
class Audit(models.Model):
    ...
    class Meta:
        ...
        indexes = [
            ...
            models.Index(fields=['user', '-action_datetime'], name='audit_user_datetime_idx'),
            # Enquiry audit trail queries
            models.Index(fields=['enquiry', '-action_datetime'], name='audit_enquiry_datetime_idx'),
        ]
...
class Contact(models.Model):
    ...
    email = models.EmailField(blank=True, help_text="Contact's email address for enquiry correspondence")
    section = models.ForeignKey(Section, related_name='contacts', on_delete=models.PROTECT, db_index=True)
    areas = models.ManyToManyField(Area, related_name='contacts')  # Changed from wards to areas
    job_types = models.ManyToManyField(JobType, related_name='contacts')  # Add this line
...
class EnquiryAttachment(models.Model):
    ...
    enquiry = models.ForeignKey(Enquiry, related_name='attachments', on_delete=models.CASCADE)
