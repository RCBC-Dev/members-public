...
class EmailProcessingService:
    ...
    def parse_email_file(
        ...
        try:
            ...
            if uploaded_file.name.lower().endswith(".msg"):
                parsed_data = parse_msg_file(
                    temp_file_path,
                    body_content_mode=parsing_mode,
    ...
    def find_member_by_email(cls, email: str) -> Optional[Member]:
        ...
        try:
            ...
            logger.warning(
                f"Multiple members found for email {email}, returning first active"
            )
            return Member.objects.filter(email__iexact=email, is_active=True).first()

    @classmethod
    def process_email_for_form_population(
    ...
    def process_email_for_history(cls, uploaded_file: UploadedFile) -> JsonResponse:
        ...
        result = cls.parse_email_file(
            uploaded_file, parsing_mode="plain", skip_attachments=True
        )
        ...
        return JsonResponse({"success": True, "email_data": processed_email})
