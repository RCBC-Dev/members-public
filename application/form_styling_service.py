...
class FormStyleService:
    ...
    @classmethod
    def apply_bootstrap_styling(cls, form_instance: forms.Form, 
                               field_mappings: Optional[Dict[str, str]] = None) -> None:
        """
        ...
        for field_name, field in form_instance.fields.items():
            ...
            bootstrap_class = field_mappings.get(field_name, cls.BOOTSTRAP_CLASSES.get(field_type, 'form-control'))
            
            # Apply Bootstrap class
            current_class = field.widget.attrs.get('class', '')
            if bootstrap_class not in current_class:
                if current_class:
                    field.widget.attrs['class'] = f"{current_class} {bootstrap_class}".strip()
                else:
                    field.widget.attrs['class'] = bootstrap_class
    
    @classmethod
    def apply_select_field_styling(cls, form_instance: forms.Form, 
                                  field_names: List[str],
                                  use_filter_labels: bool = False) -> None:
        """
        ...
        """
        empty_labels = cls.FILTER_EMPTY_LABELS if use_filter_labels else cls.EMPTY_LABELS
    ...
    @classmethod
    def apply_text_field_styling(cls, form_instance: forms.Form,
    ...
    @classmethod
    def populate_choice_fields(cls, form_instance: forms.Form,
                              use_filter_labels: bool = False) -> None:
        """
        ...
        if 'admin' in form_instance.fields:
            active_admins = [
                ...
                for a in Admin.objects.select_related('user').filter(user__is_active=True)
            ]
            inactive_admins = [
                (a.id, f"{a.user.get_full_name()}*")
                for a in Admin.objects.select_related('user').filter(user__is_active=False)
            ]
            form_instance.fields['admin'].choices = [
                ('', empty_labels.get('admin', 'Select Admin...'))
            ] + active_admins + inactive_admins
    ...
    @classmethod
    def apply_date_field_styling(cls, form_instance: forms.Form,
                                field_names: List[str],
    ...
    def _get_field_type(cls, field: forms.Field) -> str:
        ...
        if isinstance(field, forms.CharField):
            ...
            return 'email'
        elif isinstance(field, forms.IntegerField) or isinstance(field, forms.FloatField):
            return 'number'
