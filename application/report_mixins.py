...
class OverdueReportMixin(BaseReportMixin):
    ...
    def get_overdue_queryset(self, threshold_days=5):
        ...
        enquiries = Enquiry.objects.filter(status__in=["new", "open"]).select_related(
            "member", "section__department", "admin__user"
        )
        ...
        for enquiry in enquiries:
            ...
            if (
                ...
                and business_days_since_created > threshold_days
            ):
...
class CountReportMixin(BaseReportMixin):
    ...
    def get_count_data(self, model_class, filter_field, months=12):
        ...
        objects = (
            ...
            .annotate(enquiry_count=Count(filter_field))
...
class MonthlyReportMixin(BaseReportMixin):
    ...
    def _make_section_entry(self, section):
        """Create a new section data entry for SLA tracking."""
        return {
    ...
    def _classify_closed_enquiry(self, section_entry, enquiry, sla_days):
        """Classify a closed enquiry as within or outside SLA."""
        from .utils import calculate_business_days
        ...
        is_within_sla = (
            business_days_to_close is not None and business_days_to_close <= sla_days
        )
        if is_within_sla:
            section_entry["enquiries_within_sla"] += 1
        else:
            section_entry["enquiries_outside_sla"] += 1

    def _has_any_enquiries(self, section_entry):
        """Check if a section entry has any enquiries recorded."""
        return (
            section_entry["enquiries_within_sla"] > 0
            or section_entry["enquiries_outside_sla"] > 0
            or section_entry["enquiries_open"] > 0
        )
    ...
    def get_sla_sections(self, month_start, month_end, sla_days=5):
        ...
        for enquiry in month_enquiries:
            ...
            if section_id not in section_data:
                section_data[section_id] = self._make_section_entry(enquiry.section)

            entry = section_data[section_id]

            if enquiry.status == "closed" and enquiry.closed_at:
                self._classify_closed_enquiry(entry, enquiry, sla_days)
            elif enquiry.status in ["new", "open"]:
                entry["enquiries_open"] += 1

        sections = [
            data for data in section_data.values() if self._has_any_enquiries(data)
        ]
...
class EnquiryListReportMixin(BaseReportMixin):
    ...
    def get_enquiry_list(self, **filter_kwargs):
        ...
        enquiries = (
            Enquiry.objects.filter(**filter_kwargs)
            .select_related(
