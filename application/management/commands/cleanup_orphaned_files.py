...
class Command(BaseCommand):
    ...
    def handle(self, *args, **options):
        self.stdout.write(self.style.HTTP_INFO("Orphaned File Cleanup Tool"))
        self.stdout.write("=" * 60)

        # Safety check
        if not options["dry_run"] and not options["confirm"]:
            self.stdout.write(self.style.WARNING("This will permanently delete files!"))
                ...
                for file in files:
                    ...
                    try:
                        ...
                        if normalized_relative not in db_file_paths:
                            ...
                            if (
                                cutoff_date
                                and timezone.make_aware(modified_time) > cutoff_date
                            ):
                                continue
