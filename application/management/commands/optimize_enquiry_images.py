    ...
    def analyze_images(self, image_files, max_size_mb):
        ...
        for file_path in image_files:
            try:
                ...
                date_folder = next(
                    (p for p in path_parts if len(p) == 10 and p[4] == '-' and p[7] == '-'),
                    None
                )
    ...
    def cleanup_existing_backups(self, directory, dry_run=False):
        ...
        if dry_run:
            ...
            for backup_path, size_mb in backup_files[:10]:
                self.stdout.write(f"  {os.path.basename(backup_path)} ({size_mb:.1f} MB)")
            ...
            for backup_path, size_mb in backup_files:
                try:
                    ...
                    self.stdout.write(f"ERROR deleting {os.path.basename(backup_path)}: {e}")
