...
class Command(BaseCommand):
    ...
    def handle(self, *args, **options):
        self.stdout.write(
            self.style.HTTP_INFO('File Storage Analysis Tool')
        )
    ...
    def analyze_directory(self, directory):
        ...
        try:
            for root, dirs, files in os.walk(directory):
                ...
                for file in files:
                    ...
                    if file_path.is_file():
                        ...
                        self.analysis_data['file_types'][ext]['size'] += file_info['size']
            ...
            self.stdout.write(
                self.style.ERROR(f'Error analyzing {directory}: {e}')
            )
    ...
    def find_orphaned_files(self):
        ...
        self.stdout.write('NOTE: Summernote images are NOT checked as they are embedded in enquiry descriptions')
        ...
        self.stdout.write(f'Found {orphaned_count} orphaned files ({self.format_size(orphaned_size)})')
    ...
    def display_analysis(self, detailed=False):
        ...
        self.stdout.write(f'Total size: {self.format_size(self.analysis_data["total_size"])}')
        self.stdout.write('')
        ...
        for dir_path, dir_data in self.analysis_data['directories'].items():
            ...
            if detailed:
                ...
                for ext, data in sorted(dir_data['file_types'].items()):
                    self.stdout.write(f'    {ext or "no extension"}: {data["count"]} files, {self.format_size(data["size"])}')
            self.stdout.write('')
        
        # File type summary
        self.stdout.write('FILE TYPE SUMMARY')
        self.stdout.write('-' * 40)
        for ext, data in sorted(self.analysis_data['file_types'].items(), 
                               key=lambda x: x[1]['size'], reverse=True):
            percentage = (data['size'] / self.analysis_data['total_size']) * 100
            self.stdout.write(f'{ext or "no extension"}: {data["count"]} files, {self.format_size(data["size"])} ({percentage:.1f}%)')
        ...
        for category in ['tiny', 'small', 'medium', 'large', 'very_large', 'huge']:
            ...
            if count > 0:
                ...
                self.stdout.write(f'{size_labels[category]}: {count} files ({percentage:.1f}%)')
        ...
        if self.analysis_data['orphaned_files']:
            ...
            if detailed and orphaned_count > 0:
                ...
                for orphan in self.analysis_data['orphaned_files'][:20]:  # Show first 20
                    self.stdout.write(f'  {orphan["relative_path"]} ({self.format_size(orphan["size"])})')
    ...
    def export_to_csv(self, filename):
        ...
        with open(filename, 'w', newline='', encoding='utf-8') as csvfile:
            ...
            # Write header
            writer.writerow(['Directory', 'File Path', 'File Name', 'Extension', 'Size (bytes)', 'Size (formatted)', 'Modified', 'Is Orphaned'])
            
            # Write file data
            for dir_path, dir_data in self.analysis_data['directories'].items():
                dir_name = Path(dir_path).name
                orphaned_paths = {f['path'] for f in self.analysis_data['orphaned_files']}
