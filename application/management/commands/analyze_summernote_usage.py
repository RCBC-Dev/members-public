...
class Command(BaseCommand):
    ...
    def handle(self, *args, **options):
        self.stdout.write(
            self.style.HTTP_INFO('Summernote Image Usage Analysis')
        )
        ...
        if not summernote_dir.exists():
            self.stdout.write(
                self.style.WARNING('No Summernote directory found')
            )
            return
    ...
    def scan_summernote_files(self, summernote_dir):
        ...
        self.stdout.write(f'Found {self.analysis_data["total_summernote_files"]} Summernote files')
    ...
    def scan_enquiry_descriptions(self):
        ...
        summernote_pattern = re.compile(
            r'/media/django-summernote/[^"\s]+\.(jpg|jpeg|png|gif|webp)',
            re.IGNORECASE
        )
        ...
        self.stdout.write(f'Found {len(self.analysis_data["referenced_files"])} referenced images')
    ...
    def find_unused_files(self, summernote_dir, older_than_days=None):
        ...
        for root, dirs, files in os.walk(summernote_dir):
            for file in files:
                ...
                try:
                    ...
                    if normalized_relative not in self.analysis_data['referenced_files']:
        ...
        unreferenced_size = sum(f['size'] for f in self.analysis_data['unreferenced_files'])
        
        self.stdout.write(f'Found {unreferenced_count} unreferenced files ({self.format_size(unreferenced_size)})')
    ...
    def display_analysis(self):
        ...
        # Overall statistics
        self.stdout.write(f'Total Summernote files: {self.analysis_data["total_summernote_files"]:,}')
        self.stdout.write(f'Total Summernote size: {self.format_size(self.analysis_data["total_summernote_size"])}')
        self.stdout.write(f'Referenced images: {len(self.analysis_data["referenced_files"])}')
        self.stdout.write(f'Enquiries with images: {self.analysis_data["enquiries_with_images"]}')
        ...
        if self.analysis_data['total_summernote_files'] > 0:
            referenced_percentage = (len(self.analysis_data['referenced_files']) / 
                                   self.analysis_data['total_summernote_files']) * 100
    ...
    def export_to_csv(self, filename):
        ...
        with open(filename, 'w', newline='', encoding='utf-8') as csvfile:
            ...
            # Write header
            writer.writerow(['File Path', 'Size (bytes)', 'Size (formatted)', 'Modified', 'Status'])
            ...
            for file_info in self.analysis_data['unreferenced_files']:
                writer.writerow([
                    ...
                    file_info['modified'].isoformat() if file_info['modified'] else '',
                    'Unreferenced'
                ])
