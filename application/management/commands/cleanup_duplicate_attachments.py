...
class Command(BaseCommand):
    ...
    def handle(self, *args, **options):
        ...
        duplicates = (
            EnquiryAttachment.objects.values("file_path")
        ...
        if total_files_with_dupes == 0:
            self.stdout.write(
                self.style.SUCCESS("No duplicate attachment records found!")
            )
            return

        self.stdout.write(
            f"Found {total_files_with_dupes} files with duplicate records\n"
        )
        ...
        for dup in duplicates:
            ...
            attachments = (
                EnquiryAttachment.objects.filter(file_path=file_path)
            ...
            enquiry_ref = (
                first_attachment.enquiry.reference
                if first_attachment.enquiry
                else "N/A"
            )

            self.stdout.write(f"  {enquiry_ref}: {first_attachment.filename}")
            self.stdout.write(
        ...
        if dry_run:
            self.stdout.write(
                f"Would delete:                  {total_records_to_delete}"
            )
            self.stdout.write("\nRun without --dry-run to remove duplicates")
        else:
            self.stdout.write(
                self.style.SUCCESS(f"Deleted:                       {deleted_count}")
            )
