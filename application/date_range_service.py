...
class DateRangeService:
    ...
    def dates_match_predefined_range(cls, date_from_str: str, date_to_str: str) -> bool:
        ...
        try:
            ...
            for range_type in ["3months", "6months", "12months"]:
                date_range_info = get_preset_date_range(
                    range_type, timezone_aware=False
                )
                expected_from = datetime.strptime(
                    date_range_info.date_from_str, "%Y-%m-%d"
                ).date()
                expected_to = datetime.strptime(
                    date_range_info.date_to_str, "%Y-%m-%d"
                ).date()

                # Allow tolerance for "today" since it might change between requests
                if (
                    abs((date_from - expected_from).days) <= cls.DATE_MATCH_TOLERANCE
                    and abs((date_to - expected_to).days) <= cls.DATE_MATCH_TOLERANCE
                ):
                    return True
    ...
    def get_default_filter_params(
        cls, status: str = "", date_range: str = "12months"
    ) -> Dict[str, str]:
        """
        ...
        if date_range in cls.PRESET_RANGES and date_range != "all":
            ...
            return {"status": status, "date_range": date_range}

    @classmethod
    def get_default_filter_redirect(
        cls, request_path: str, status: str = "", date_range: str = "3months"
        ...
        if date_range == "custom":
            ...
            if date_from_custom:
                date_from_dt = timezone.make_aware(
                    datetime.combine(date_from_custom, time.min)
                )
                queryset = queryset.filter(created_at__gte=date_from_dt)
            if date_to_custom:
                date_to_dt = timezone.make_aware(
                    datetime.combine(date_to_custom + timedelta(days=1), time.min)
                )
