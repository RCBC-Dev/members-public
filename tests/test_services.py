...
from application.models import (
    ...
    Enquiry, EnquiryHistory, Contact, EnquiryAttachment
)
from application.services import EnquiryService, EnquiryFilterService, EmailProcessingService, MemberService
...
class TestEnquiryService:
    ...
    def test_create_enquiry_with_attachments_with_images(self):
        ...
        history_entries = EnquiryHistory.objects.filter(enquiry=enquiry).order_by('created_at')
        ...
        assert 'Enquiry created with 2 file(s) from email: test1.jpg, test2.png' == history_note
    ...
    def test_process_extracted_images_invalid_json(self):
        ...
        result = EnquiryService._process_extracted_images(
            extracted_images_json='invalid json',
            enquiry=enquiry,
            user=self.admin_user
        )
    ...
    def test_process_extracted_images_missing_fields(self):
        ...
        assert result == {'email': 1, 'manual': 0, 'total': 1, 'filenames': ['test.jpg']}
...
This is the previous email in the conversation."""
        ...
        result = EmailProcessingService.extract_latest_email_from_conversation(email_body)
...
This is a much longer previous email that should be returned as fallback when the extracted portion is too short to be meaningful."""
    ...
    def test_clean_html_for_display_whitespace(self):
        ...
        lines = result.split('\n')
        assert all(line.strip() == line for line in lines if line)  # No leading/trailing whitespace
    ...
    def test_process_email_for_history(self):
        ...
        assert 'From: old@example.com' in result['full_conversation']  # Full conversation preserved
...
class TestEnquiryFilterService:
    ...
    def test_get_optimized_queryset_with_search(self):
        ...
        assert not queryset.query.deferred_loading[0] or 'description' not in queryset.query.deferred_loading[0]
