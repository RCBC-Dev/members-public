...
class AttachmentUploadTestCase(TestCase):
    ...
    def test_attach_file_to_open_enquiry(self):
        ...
        response = self.client.post(
            url, {"file": test_file, "enquiry_id": self.open_enquiry.id}
        )
    ...
    def test_attach_file_to_closed_enquiry(self):
        ...
        response = self.client.post(
            url, {"file": test_file, "enquiry_id": self.closed_enquiry.id}
        )
    ...
    def test_attach_unsupported_file_type(self):
        ...
        test_file = SimpleUploadedFile(
            "test.txt", b"text file content", content_type="text/plain"
        )
        ...
        )  # api_upload_photos returns 200 with success: false
        data = response.json()
