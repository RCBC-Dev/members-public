...
class CSPMiddlewareTest(TestCase):
    ...
    def test_csp_nonce_present_in_headers_and_tags(self):
        ...
        try:
            ...
            self.skipTest("Could not reverse 'application:welcome'. Ensure it's defined and accessible, or update test URL.")
            return

        response = self.client.get(test_url)
        self.assertEqual(response.status_code, 200, f"Failed to fetch {test_url}. Status: {response.status_code}")
        ...
        nonce_match = re.search(r"script-src[^-]*'nonce-([a-zA-Z0-9+/=_-]+)'", csp_header)
        self.assertIsNotNone(nonce_match, f"CSP Nonce not found for script-src in header: {csp_header}")
        nonce_from_header = nonce_match.group(1)
        self.assertTrue(len(nonce_from_header) > 16, "CSP Nonce from header seems too short.")
        ...
        inline_script_matches = re.finditer(r'<script([^>]*)>(.*?)</script>', html_content, re.DOTALL)
        ...
        for match in inline_script_matches:
            ...
            if 'document.addEventListener' in script_content and 'showAlert' in script_content and 'message.tags' in script_content:
                messages_script_exists_heuristic = True
                nonce_attr_match = re.search(r'nonce=["\']([a-zA-Z0-9+/=_-]+)["\']', attrs_str)
                if not nonce_attr_match or nonce_attr_match.group(1) != nonce_from_header:
                    found_messages_script_without_correct_nonce = True
                    self.fail(f"Inline messages script found but nonce is missing or incorrect. Expected: {nonce_from_header}, Attrs: {attrs_str}")
                break
    ...
    def test_csp_middleware_adds_nonce_to_request_and_context(self):
        ...
        nonce_value_from_request = str(request.csp_nonce) # forces SimpleLazyObject to evaluate
        self.assertTrue(len(nonce_value_from_request) > 16, "Generated nonce from request.csp_nonce is too short.")
        ...
        response_to_process = TemplateResponse(request, template="some_template.html", context={})
        ...
        processed_response = middleware.process_template_response(request, response_to_process)
        self.assertIn('csp_nonce', processed_response.context_data,
                        "csp_nonce not found in template response context_data.")
        self.assertEqual(processed_response.context_data['csp_nonce'], nonce_value_from_request,
