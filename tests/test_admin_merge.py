...
from application.models import (
    Contact, JobType, Enquiry, Section, Department, Member, Ward, Admin
)
from application.admin import ContactAdmin, JobTypeAdmin, MemberAdmin, merge_contacts, merge_job_types, merge_members
...
class TestContactMerge:
    ...
    def test_merge_contacts_wrong_count(self):
        ...
        queryset = Contact.objects.filter(id__in=[self.contact1.id, self.contact2.id, contact3.id])
        merge_contacts(self.contact_admin, self.request, queryset)
...
class TestJobTypeMerge:
    ...
    def setup_method(self):
        ...
        self.enquiry1 = Enquiry.objects.create(
            title='Test Enquiry 1',
            member=self.member,
            job_type=self.job_type1
        )
        self.enquiry2 = Enquiry.objects.create(
            title='Test Enquiry 2',
            member=self.member,
            job_type=self.job_type2
        )
    ...
    def test_merge_job_types_wrong_count(self):
        ...
        queryset = JobType.objects.filter(id__in=[self.job_type1.id, self.job_type2.id, job_type3.id])
        merge_job_types(self.job_type_admin, self.request, queryset)
...
class TestMemberMerge:
    ...
    def test_merge_members_wrong_count(self):
        ...
        queryset = Member.objects.filter(id__in=[self.member1.id, self.member2.id, member3.id])
        merge_members(self.member_admin, self.request, queryset)
