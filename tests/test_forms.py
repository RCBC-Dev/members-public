...
from application.forms import (
    BaseFormHelper, EnquiryForm, StaffEnquiryForm, 
    EnquiryHistoryForm, EnquiryFilterForm
)
from application.models import (
    Admin, Area, Department, Section, Ward, Member, JobType, 
    Enquiry, EnquiryHistory, Contact
)
...
class TestBaseFormHelper:
    ...
    def test_base_form_helper_initialization(self):
        ...
        assert helper.help_text_inline is True or helper.help_text_inline is False  # Either is acceptable
...
class TestEnquiryForm:
    ...
    def test_enquiry_form_meta_configuration(self):
        ...
        assert form.Meta.fields == ['title', 'description', 'member', 'section', 'contact', 'job_type']
...
class TestEnquiryFilterForm:
    ...
    def test_enquiry_filter_form_initialization(self):
        ...
        assert len(form.fields['member'].choices) > 1  # Should have "All Members" + actual members
        assert len(form.fields['admin'].choices) > 1  # Should have "All Admins" + actual admins
        assert len(form.fields['section'].choices) > 1  # Should have "All Sections" + actual sections
        assert len(form.fields['job_type'].choices) > 1  # Should have "All Job Types" + actual job types
        assert len(form.fields['contact'].choices) > 1  # Should have "All Contacts" + actual contacts
        assert len(form.fields['ward'].choices) > 1  # Should have "All Wards" + actual wards
    ...
    def test_enquiry_filter_form_field_attributes(self):
        ...
        assert form.fields['search'].widget.attrs['maxlength'] == 100 or form.fields['search'].widget.attrs['maxlength'] == '100'
    ...
    def test_enquiry_filter_form_choice_ordering(self):
        ...
        member_choices = [choice for choice in form.fields['member'].choices if choice[0] != '']
