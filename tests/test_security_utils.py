...
class TestSecurityValidator(TestCase):
    ...
    def test_xsl_injection_detection(self):
        ...
        for malicious_input in malicious_inputs:
            ...
            assert (
                result is True
            ), f"Failed to detect XSL injection in: {malicious_input}"
    ...
    def test_path_traversal_detection(self):
        ...
        for malicious_input in malicious_inputs:
            ...
            assert (
                result is True
            ), f"Failed to detect path traversal in: {malicious_input}"
    ...
    def test_command_injection_detection(self):
        ...
        for malicious_input in malicious_inputs:
            result = SecurityValidator.contains_command_injection_pattern(
                malicious_input
            )
            assert (
                result is True
            ), f"Failed to detect command injection in: {malicious_input}"

    def test_multiple_pattern_detection(self):
        """Test detection across multiple categories."""
        test_input = (
            "<?xml version='1.0'?><xsl:template>'; DROP TABLE users;--</xsl:template>"
        )
