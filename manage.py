...
def main():
    ...
    if not settings_module:
        if environment == 'production':
            ...
            settings_module = 'project.settings.development'
            if environment:
                print(f"Warning: Unrecognized ENVIRONMENT '{environment}', defaulting to development settings.")
        os.environ['DJANGO_SETTINGS_MODULE'] = settings_module
        print(f"No DJANGO_SETTINGS_MODULE set. Using ENVIRONMENT variable: {environment.upper() or 'DEVELOPMENT'}")
    ...
    for key in [
        'DBNAME', 'DBUSER', 'DBHOST', 'DBPORT', 'DJANGO_SECRET_KEY', 'ENV', 'DEBUG', 'ALLOWED_HOSTS', 'STATIC_ROOT'
    ]:
