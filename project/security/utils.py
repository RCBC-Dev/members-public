...
class SecurityValidator:
    ...
    def contains_xsl_injection_pattern(value: str) -> bool:
        ...
        return SecurityValidator._check_patterns(
            value, SecurityPatterns.XSL_INJECTION_PATTERNS
        )
    ...
    def contains_sql_injection_pattern(value: str) -> bool:
        ...
        return SecurityValidator._check_patterns(
            value, SecurityPatterns.SQL_INJECTION_PATTERNS
        )
    ...
    def contains_path_traversal_pattern(value: str) -> bool:
        ...
        return SecurityValidator._check_patterns(
            value, SecurityPatterns.PATH_TRAVERSAL_PATTERNS
        )
    ...
    def contains_command_injection_pattern(value: str) -> bool:
        ...
        return SecurityValidator._check_patterns(
            value, SecurityPatterns.COMMAND_INJECTION_PATTERNS
        )

    @staticmethod
    def contains_any_suspicious_pattern(
        value: str, pattern_categories: Optional[List[str]] = None
    ) -> Dict[str, bool]:
        """
        ...
        for category in pattern_categories:
            if category in all_patterns:
                results[category] = SecurityValidator._check_patterns(
                    value, all_patterns[category]
                )
            else:
        ...
        try:
            for pattern in patterns:
                if re.search(pattern, value, re.IGNORECASE):
                    logger.debug(
                        f"Security pattern matched: '{pattern}' in value: {value[:100]}..."
                    )
                    return True
            return False
...
class SecurityLogger:
    ...
    def log_blocked_request(
        attack_type: str, value: str, request_path: str, user_ip: Optional[str] = None
    ) -> None:
        """
...
class RequestSecurityUtils:
    ...
    @staticmethod
    def _check_parameters_for_threats(params, param_type, pattern_categories, request):
        """Check a set of request parameters for security threats.

        Args:
            params: QueryDict of parameters to check
            param_type: Label for error messages (e.g. "GET" or "POST")
            pattern_categories: List of pattern categories to check
            request: Django request object

        Returns:
            Error message string if threats detected, None otherwise
        """
        for key, value in params.items():
            if not isinstance(value, str):
                continue
            threats = SecurityValidator.contains_any_suspicious_pattern(
                value, pattern_categories
            )
            ...
            return (
                f"Invalid request parameters detected in {param_type} parameter '{key}'"
            )
        return None
    ...
    def validate_request_parameters(
        ...
        if check_get:
            result = RequestSecurityUtils._check_parameters_for_threats(
                request.GET, "GET", pattern_categories, request
            )
            if result:
                return result
        ...
        if check_post and request.method == "POST":
            result = RequestSecurityUtils._check_parameters_for_threats(
                request.POST, "POST", pattern_categories, request
            )
            if result:
                return result
