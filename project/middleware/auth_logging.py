...
class AuthLoggingMiddleware(MiddlewareMixin):
    ...
    def process_response(self, request, response):
        ...
        if any(request.path.startswith(path) for path in self.auth_paths):
            ...
            if request.path.startswith('/accounts/login/') or request.path.startswith('/accounts/microsoft/'):
                if response.status_code == 302 and request.user.is_authenticated:  # Successful login typically redirects
                    logger.info(f"Successful authentication: {json.dumps(log_data)}")
                elif response.status_code == 400:  # Bad request might indicate attack attempt
                    logger.warning(f"Rejected authentication attempt: {json.dumps(log_data)}")
    ...
    def _sanitize_params(self, params_dict):
        ...
        sensitive_params = ['password', 'token', 'access_token', 'id_token', 'refresh_token', 'secret']
